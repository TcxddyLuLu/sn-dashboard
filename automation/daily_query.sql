-- Daily Breakdown: Tickets per employee per closed date (CST) for current month
...
FROM (
  SELECT employee_id, closed_date, cnt AS incident_count, 0 AS task_count FROM incident_daily
  UNION ALL
  SELECT employee_id, closed_date, 0 AS incident_count, cnt AS task_count FROM task_daily
) combined
GROUP BY employee_id, closed_date
ORDER BY closed_date, employee_id
